...
Sw_0202_loss::Sw_0202_loss(int csv_type) :Cal_model(csv_type)
{
	m_src_type = csv_type;
	m_cal_data = (T_sw_0202 *)App_lib::app_malloc(MAX_CHANNEL_COUNT * sizeof(T_sw_0202));
	init_freq();
}

Sw_0202_loss::~Sw_0202_loss()
{
	if(m_cal_data != NULL)
	{
	    delete m_cal_data;
...
void Sw_0202_loss::calibration_data_reset()
{
	memset(m_cal_data, 0, MAX_CHANNEL_COUNT * sizeof(T_sw_0202));
	init_freq();
}

int Sw_0202_loss::wt_calibration_initial(char *buf)
{
	int len = 0;
	int unit = 0;
	int ret = ERR_RET_OK;
	do
	{
		len = p_csv_manager->csv_file_get("SW0202/sw_1_0202_insertion_loss.csv", buf);
		ret = get_sw0202_from_buf(buf, len, unit);
		err_break(ret);
		fill_cal_data(unit);
		unit++;

		len = p_csv_manager->csv_file_get("SW0202/sw_2_0202_insertion_loss.csv", buf);
		ret = get_sw0202_from_buf(buf, len, unit);
		err_break(ret);
		fill_cal_data(unit);
		unit++;

		len = p_csv_manager->csv_file_get("SW0202/sw_3_0202_insertion_loss.csv", buf);
		ret = get_sw0202_from_buf(buf, len, unit);
		err_break(ret);
		fill_cal_data(unit);
		unit++;

		len = p_csv_manager->csv_file_get("SW0202/sw_4_0202_insertion_loss.csv", buf);
		ret = get_sw0202_from_buf(buf, len, unit);
		err_break(ret);
		fill_cal_data(unit);
	}while(0);
...
double Sw_0202_loss::get_calibration_data(char *ref_buf)
{
	return 0;
}

int Sw_0202_loss::get_sw0202_from_buf(char *file_buf, int len, int unit)
{
...
int Sw_0202_loss::fill_cal_data(int unit)
{
	int i = 0;
	int j = 0;
	int freq_index1 = 0;
