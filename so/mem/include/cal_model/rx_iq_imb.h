...
#ifndef INCLUDE_CAL_MODEL_RX_IQ_IMB_H_
#define INCLUDE_CAL_MODEL_RX_IQ_IMB_H_
...
typedef struct rx_iq_imb_data
{
	int freq;	    /* 频率 索引 */
	BOOL flag[UNIT_NUM];   /* 校准数据填充标记 */
	Rx_Iq_Imb_Parm rx_iq_imb[UNIT_NUM];
} T_rx_iq_imb;

class Rx_iq_imb : public Cal_model
{
private:
	T_rx_iq_imb * m_cal_data;
	map<int, int> m_cal_map;  //5G 频段有多个中频， 为方便插值，引入stl map结构体保存校准索引数据, <freq, value>

public:
	Rx_iq_imb(int csv_type);
	virtual ~Rx_iq_imb();
...
private:
	...
	int get_iq_imb_from_buf(char *file_buf, int len, int unit);
...
#endif /* INCLUDE_CAL_MODEL_RX_IQ_IMB_H_ */
